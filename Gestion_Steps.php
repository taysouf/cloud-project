...
<div id="messageInfo"></div>
      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        <li class="treeview menu-open active">
          ...
          <ul class="treeview-menu">
            ...
            <li class="active"><a href="Gestion_Steps.php"><i class="fa fa-file"></i> Gestion des Steps</a></li>
          </ul>
        </li>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Gestion des Projets
        <br><small>Liste des Steps</small>
      </h1>
    </section>
    ...
    <section class="content">
      <div class="row"> <!-- la class row sert à ligner deux div ou plus dans la même ligne -->
        ...
        <div class="col-md-12">
             ...
             <button type="button" title="Ajouter Step" class="btn btn-success AjouterStep pull-right" data-toggle="modal" data-target="#addStep"  ><i class="fa fa-plus" ></i></button>
                ...
                <?php
                  ...
                  <td> <button id="<?php echo $stepId ?>" class="ModifierStep btn-xs btn btn-primary"  data-toggle="modal" data-target="#ModifierStepModal" data-designation="<?php echo $designation ?>" data-value="<?php echo $stepId ?>" type="button"><i class="fa fa-edit" aria-hidden="true"></i></button></td>
                  <td><button id="<?php echo $stepId ?>" class="DeleteStep btn-xs btn btn-danger" data-value="<?php echo $stepId ?>"><i class="fa fa-times" aria-hidden="true"></i></button></td>
...
<div class="modal fade" id="addStep" role="dialog" >
  ...
  <div class="modal fade" id="ModifierStepModal" role="dialog" >
    <div class="modal-dialog ">
      ...
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">Modifier Step</h4>
        </div>
        <div class="modal-body">
        <form id="form-ModifierStep">
                <div class="form-group">
                    <label for="email">Description du Step :</label>
                    <input type="hidden" name="stepIdModifier" id="stepIdModifier">
                    <input required="true" type="text" class="form-control" id="NomStepModifer" placeholder="Entrer la description du Step" name="NomStepModifer" >
                </div>
         </form>
 
        </div>  
        <div class="modal-footer">
          <button style="width: 150px;" id="btn-ModifierStep" class="btn btn-success col-sm-offset-3">Enregistrer</button>
          <button type="button" class="btn btn-danger col-sm-offset-1" id="close" data-dismiss="modal">Close</button>
        </div>
      </div>
...
<script type="text/javascript">
 ...
 if (confirm('Êtes-vous sûr de Supprimer ce projet?')) {
   $.ajax({
      type:"POST",
      url:"projetAjax.php",
      data:"DeleteStep="+del_id,
      success:function(data) {
          
          new PNotify({
            title: 'Suppression réussi',
            text: "Step supprimé avec succes !!",
            type: 'success',
            icon: 'fa fa-check',
            styling: "bootstrap3",
            delay: 1800
...
$('#btn-ajouterStep').click(function(){
    ...
    $(".ModifierStep").click(function () {
    var stepId=$(this).data('value');
    var designation=$(this).data('designation');

    $("#stepIdModifier").val(stepId);
    $("#NomStepModifer").val(designation);
    });

$('#btn-ModifierStep').click(function(){

        $.ajax({
      type: "POST",
      url:  "projetAjax.php",
      data: $("#form-ModifierStep" ).serialize(),
      success: function(result){
        $('#close').click();
        location.reload();
      }
